...
app.get('/api/articles/:id', async (req,res) => { try { const articles = await getArticles(1, 'all'); const article = articles.find(a => a.id === req.params.id); article ? res.json(article) : res.status(404).json({ error:'Article not found' }) } catch { res.status(502).json({ error:'Unable to retrieve the article.' }) } })
